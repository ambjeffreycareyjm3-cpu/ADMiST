...
class SCBCCosmology:
    ...
    def growth_function(self, z):
        ...
        return 2.5 * Omega_m_z * a / (Omega_m_z**(4/7) - (1 - Omega_m_z) + (1 + Omega_m_z/2)*(1 + (1 - Omega_m_z)/70))
    ...
    def plot_predictions(self, out_png='ADMST_predictions_min.png'):
        ...
        ax.axvline(self.params.k_J, color='gray', ls='--', label=f'k_J={self.params.k_J}')
        ...
        ax2.set_xlabel('f [Hz]'); ax2.set_ylabel('h^2 Omega_GW')
        ax2.grid(True)
