...
class SCBCCosmology:
    ...
    def cmb_damping_modulation(self, ell):
        ...
        return np.exp(-((ell / ell_D) ** 1.7)) * (
            1 + eps * np.cos(2 * np.pi * ell / self.params.ell_osc)
        )
    ...
    def bbn_neff(self):
        ...
        delta_Neff = (
            (4.0 / 7.0)
            * (delta_g / 2.0)
            * (self.params.T_star * 1000.0 / T_nu) ** 4
            * np.exp(-4.0)
        )
    ...
    def plot_predictions(self, out_png="ADMST_corrected_predictions.png"):
        ...
        ax1.set_title(
            f"Matter Power Spectrum ($\\sigma_8^{{ADMST}}={sigma8_ADMST:.3f}$)"
        )
        ...
        ax2.axvline(
            ...
            ls="--",
            alpha=0.5,
            label=f"ell_osc={self.params.ell_osc}",
        )
        ...
        ax3.axvline(
            self.params.f_p, color="b", ls="--", label=f"{self.params.f_p*1e3:.1f} mHz"
        )
        ax3.axvline(
            self.params.f_s, color="r", ls="--", label=f"{self.params.f_s*1e3:.1f} mHz"
        )
